...
<html lang="en">
    ...
    <body>
        <nav class="navbar navbar-inverse navbar-fixed-top">
            <div class="container-fluid">
                <div class="navbar-header">
                    ...
                    <a class="navbar-brand" href="#">GUES - Clientes</a>
                </div>
        ...
        <div class="container-fluid">            
            <div class="row">
                ...
                <div class="col-xs-12 main">
                    ...
                    <div class="row">
                        <div class="col-xs-6">
                            <a type="button" class="btn btn-default btn-block active" href="clientes-precios.php">Clientes</a>
                        </div>
                        <div class="col-xs-6">                    
                            <a type="button" class="btn btn-default btn-block" href="provedores-precios.php">Provedores</a>
                        </div>
                    </div>
                    <br>
                        ...
                        <table class="table table-striped item-table add">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td class="cliente"><input type="text" class="form-control"/></td>
        ...
        <script>
            ...
            //COLUMNAS
            var columnas = ['Producto', 'Cliente', 'Precio', 'Moneda', 'Unidad', 'Fecha'];
            ...
            //CLIENT BUTTONS
            function AddClientButtons(data) {
                var clientDiv = $("#client-div").html("");
                var uniqueClients = GetUniques(data, "Cliente").sort();
                
                $.each(uniqueClients, function(i, client) { 
                    var $div = $('<div class="col-xs-3">');
                    var $button = $('<input type="button" class="btn btn-info btn-block">').val(client);
                    $($div).append($button);
                    $(clientDiv).append($div);
                });
                
                $('#client-div div input').on('click', function() {
                    $('#client-div div input').removeClass("active")
                    $(this).addClass("active");
                    var cliente = $(this).val();                    
                    //TABLE
                    FilterTable("cliente", cliente);
                    //GRAPHIC
                    currDataset = dataset.filter(function(obj) { return obj.Cliente == cliente });
                    $("div .graph").html("");
                    AddProductButtons(currDataset);
                });
            }
            ...
            function AddProductButtons(data) {
                ...
                $('#product-div div input').on('click', function() {
                    ...
                    //GRAPHIC
                    var data = currDataset.filter(function(obj) { return obj.Producto == producto });
                    console.log(data);
            ...
            function LoadData(sortFunction, field) {
                $.getJSON( "clientes-data.json", function( data, error ) {
            ...
            function BuildTable(data) {
                ...
                $.each(data, function(i, item) {        
                    var $tr = $('<tr>').append(
                        ...
                        $('<td>').addClass("cliente").append($('<input type="text" class="form-control">').val(item.Cliente)),
            ...
            function GetDataSeries(data) {
                var clientes = GetUniques(data, "Cliente").sort();
            ...
            function GenerateGraphic(data) {
                ...
                var clientes = color.domain().map(function(name) {
                    return {
                        Cliente: name,
                ...
                x.domain([
                    d3.min(clientes, function(c) { return d3.min(c.Valores, function(v) { return v.Fecha; }); }),
                    d3.max(clientes, function(c) { return d3.max(c.Valores, function(v) { return v.Fecha; }); })
                ]);
                y.domain([
                    0,
                    d3.max(clientes, function(c) { return d3.max(c.Valores, function(v) { return v.Precio; }); })
                ]);            
                ...
                var clienteLinea = svg.selectAll(".clienteLineGroup")
                    .data(clientes)
                    .enter().append("g")
                    .attr("class", ".clienteLineGroup");
                
                clienteLinea.append("path")
                    .attr("class", "line")
                    .attr("d", function(d) { return line(d.Valores); })
                    .style("stroke", function(d) { return color(d.Cliente); });                
                
                //PUNTOS
                clientesDot = [];
                $.each(clientes, function(i, cliente) {
                    $.each(cliente.Valores, function(j, valores) {
                       if (!isNaN(valores.Precio)) {
                           clientesObj = {};
                           clientesObj.Cliente = cliente.Cliente;
                           clientesObj.Fecha = valores.Fecha;
                           clientesObj.Precio = valores.Precio;
                           clientesDot.push(clientesObj);
                       }
                    });
                });

                var clientePunto = svg.selectAll(".clienteDotGroup")
                    .data(clientesDot)
                    .enter().append("g")
                    .attr("class", ".clienteDotGroup");
                
                clientePunto.append("circle")
                    .attr("class", "dot")
                    .attr("cx", function(d) { return x(d.Fecha); })
                    .attr("cy", function(d) { return y(d.Precio); })
                    .style("fill", function(d) { return color(d.Cliente); })
                    .attr("r", 3.5); 
